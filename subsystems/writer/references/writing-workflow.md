...
- 写结构物件 → 加载 `references\craft\writing-craft.md` ✅/❌
- 写反派/揭露方式 → 加载 `villain-and-reveal.md` ✅/❌
- 写大纲 → 已有 references\craft\genre-writing-formulas.md 的题材速查表即可 ✅/❌
- 反转信息差验证 → 需加载 `references\craft\reversal-toolkit.md` 或 `references\craft\writing-craft.md` ✅/❌
- 伏笔回查 → 需加载 `references\craft\writing-craft.md` ✅/❌
- 如某项不适用（如无反派），标注原因后可跳过

### 设计步骤

1. **设计结构物件**（1-2 个）：用 references\craft\writing-craft.md「结构物件系统」模板
...
4. **编写 小节大纲.md**：每节 1 行，格式如下：
   ...
   - 子事件不足 3 个时用扩展规则（见 references\craft\writing-craft.md 第 7 节「子事件不够时怎么扩」）
...
- 必读：`references\craft\format-and-structure.md`
- 必读：`references\craft\anti-ai-writing.md`（写作时自检 AI 腔）
- 按题材加载 1 个：`references\craft\genre-writing-formulas.md` 中对应题材速查表
- 按需加载：`references\craft\writing-craft.md` / `villain-and-reveal.md` / `references\craft\emotional-methods.md`
...
- [ ] 完整 AI 腔黑名单见 references\craft\anti-ai-writing.md
